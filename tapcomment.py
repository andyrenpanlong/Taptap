...
def search_all_page_url():
    start_url = 'https://www.taptap.com/app/14065/review'
...
def search_content(url):
    ...
    if r.status_code == 200:
        ...
        for i in commentList:
            ...
            gameObj["comment_micro_time"] = i.select(".text-header-time span")[0].get("data-dynamic-time")   #评论时间（毫秒数）
            ...
            gameObj["comment_score"] = i.select(".item-text-score .colored")[0].get("style").replace("width: ", "").replace("px", "")  #评论得分
            gameObj["comment_replay_num"] = i.select(".question-witch-replay .normal-text")[0].text.replace("回复", "").replace("(", "").replace(")", "") or str(0)   #点赞数量
